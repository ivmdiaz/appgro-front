...
@Component({
  ...
  templateUrl: './reporte-adm-tiendas.component.html',
  styleUrls: ['./reporte-adm-tiendas.component.scss']
})
export class ReporteAdmTiendasComponent implements OnInit {
  ...
  @ViewChild('fechaHastaPicker') fechaHastaPicker: MatDatepicker<Date>;
  ...
  obtenerTiendasBd(): void {
    this.reporteService.obtenerTiendasAdministrador().subscribe(
      data => {
        this.tiendasBd = data;
        this.filtroTiendas();
      },
  ...
  private _createFilters(): FiltroModel[] {
    ...
    fechaVenta.setFilterDateLessThan("fechaVenta", moment(this.f.fechaHasta.value).format('YYYY-MM-DD'));
